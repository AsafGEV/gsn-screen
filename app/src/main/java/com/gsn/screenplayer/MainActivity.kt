...
class MainActivity : AppCompatActivity() {
    ...
    override fun onWindowFocusChanged(h: Boolean) { super.onWindowFocusChanged(h); if (h) hideSystem() }
    ...
    // Q = double-quote char; used to build JSON/strings without backslash escaping
    private fun j(k: String, v: String) = Q + k + Q + ":" + Q + v + Q
    private fun jobj(vararg pairs: String) = "{" + pairs.joinToString(",") + "}"
    private fun jraw(k: String, v: String) = Q + k + Q + ":" + v
    ...
    private fun beat() {
        ...
        thread {
            try {
                ...
                val data = jobj(j("device_id", deviceId), j("last_seen", now), j("app_version", APP_VER))
                if (hbRecordKey.isEmpty()) {
                    val body = jobj(j("app", HB_TABLE), jraw("data", data))
                    ...
                    val body = jobj(j("app", HB_TABLE), j("record_key", hbRecordKey), jraw("data", data))
    ...
    private fun parseVal(json: String, key: String): String {
        val marker = Q + key + Q + ":" + Q
        val i = json.indexOf(marker)
        if (i < 0) return ""
        val s = i + marker.length
        val e = json.indexOf(Q.toString(), s)
    ...
    private fun httpPost(urlStr: String, body: String): String {
        return try {
            ...
            c.doOutput = true; c.connectTimeout = 15000; c.readTimeout = 15000
